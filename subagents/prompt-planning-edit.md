...
- Then act proportionately. If execution would fail or require rework, fix every real problem in a single pass. If the plan would execute successfully and land the user's requested end state, declare it already excellent — even if you could imagine different wording, finer task splits, or template improvements.
...
- Consider whether the plan is solving the right problem. If a fundamentally better direction exists, rewrite in part or from scratch. Prefer changing direction over incrementally repairing a plan that is on the wrong track.
