...
- Read the current implementation plan critically.
- Critique the current implementation plan for the user's request based on its adherence to the task_input_json (which overrides all other concerns) and its adherence to the principles taught in `trycycle-planning`. You are the arbiter of excellence, and also the one responsible for fixing anything you deem less than excellent.
- If the current plan is already excellent, declare it already excellent unchanged. If it is not, improve it directly so that the resulting plan is excellent.
