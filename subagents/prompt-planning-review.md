...
- A critical plan issue is a current-work issue: a flaw, ambiguity, missing invariant, weak architecture, or materially better approach that affects whether the user vision will be realized well.
- Do not report critical issues for later work. File later work with `<file_later_work_command>` and leave it out of `## Findings memo`.
