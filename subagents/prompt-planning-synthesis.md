...
- If the current implementation plan contains work that is not strictly necessary to accomplish the user's current request, remove it from the revised plan and file it as later work. Do not preserve scoped-out work just because it is valuable.
- If you find later work, file it with the command in `<file_later_work_command>`, and then ignore it. Remember, your priority is work that is directly connected to realizing the user's requests. Do not include filed later work in the revised plan, synthesis summary, blocker lists, plan edits, or implementation targets.
